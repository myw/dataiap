...
def walk_emails(folder):
    ...
    terms_per_email = defaultdict(set)
    ...
    for e in EmailWalker(folder):
        ...
        terms_per_email[id(e)].update(terms_in_email)

    # Count all the emails that every term appears in
    allterms = Counter()
    for terms in terms_per_email.itervalues():
        allterms.update(terms)

    # From the counts, compute the IDFs
    idfs = {}
    nemails = len(terms_per_email) # num of keys is num of folders
    for term, email_count in allterms.iteritems():
        idfs[term] = math.log(nemails / (1.0 + email_count))
